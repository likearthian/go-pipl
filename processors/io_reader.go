...
func (r *IoReader) ProcessData(d data.Data, outputChan chan data.Data, killChan chan error) {
	if r.Gzipped {
		gzReader, err := gzip.NewReader(r.Reader)
		util.KillPipelineIfErr(err, killChan)
		r.Reader = gzReader
	}
	r.ForEachData(killChan, func(d data.Data) {
		outputChan <- d
	})
}

// Finish - see interface for documentation.
func (r *IoReader) Finish(outputChan chan data.Data, killChan chan error) {
}
...
func (r *IoReader) ForEachData(killChan chan error, foo func(d data.Data)) {
	if r.LineByLine {
...
func (r *IoReader) scanLines(killChan chan error, forEach func(d data.Data)) {
	scanner := bufio.NewScanner(r.Reader)
	for scanner.Scan() {
		forEach(data.FromRawBytes([]byte(scanner.Text())))
	}
	err := scanner.Err()
	util.KillPipelineIfErr(err, killChan)
}

func (r *IoReader) bufferedRead(killChan chan error, forEach func(d data.Data)) {
