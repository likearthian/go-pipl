...
import (
	...
	"github.com/likearthian/go-pipl/logger"
...
func (p *Pipeline) connectStages() {
	...
	for _, stage := range p.layout.stages {
		for _, from := range stage.processors {
			if from.outputs != nil {
				from.branchOutChans = []chan data.Data{}
				for _, to := range p.dataProcessorOutputs(from) {
					if to.mergeInChans == nil {
						to.mergeInChans = []chan data.Data{}
					}
					c := p.initDataChan()
		...
		for _, dp := range stage.processors {
			...
			go func(n int, dp *dataProcessor) {
				...
				for d := range dp.inputChan {
					logger.Debug(p.Name, "- stage", n+1, dp, "received data")
					if p.PrintData {
						logger.Debug(p.Name, "- stage", n+1, dp, "data =", fmt.Sprintf("%#v", d))
					}
...
func (p *Pipeline) Run() (killChan chan error) {
	...
	for _, dp := range p.layout.stages[0].processors {
		...
		dp.inputChan <- data.FromRawBytes([]byte(StartSignal))
		dp.Finish(dp.outputChan, killChan)
		close(dp.inputChan)
	}
...
func (p *Pipeline) initDataChans(length int) []chan data.Data {
	cs := make([]chan data.Data, length)
	for i := range cs {
		cs[i] = p.initDataChan()
	}
	return cs
}
func (p *Pipeline) initDataChan() chan data.Data {
	return make(chan data.Data, p.BufferLength)
}
...
func (p *Pipeline) WithBufferLength(length int) *Pipeline {
	cp := *p
	cp.BufferLength = length
	return &cp
}

func (p *Pipeline) WithPrintData() *Pipeline {
	cp := *p
	cp.PrintData = true
	return &cp
}
